...
// tools/scripts/polygraph/background-session-mcp.ts
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.20.2/node_modules/@modelcontextprotocol/sdk/dist/esm/server/index.js
...
// node_modules/.pnpm/@modelcontextprotocol+sdk@1.20.2/node_modules/@modelcontextprotocol/sdk/dist/esm/server/stdio.js
import process2 from "node:process";

// node_modules/.pnpm/@modelcontextprotocol+sdk@1.20.2/node_modules/@modelcontextprotocol/sdk/dist/esm/shared/stdio.js
...
var tools = [
  {
    name: "background_session_start",
    description: "Fail-closed bootstrap for one provider-managed background Polygraph session. Invoke before repository work.",
    inputSchema: {
      type: "object",
      properties: {
        task: {
          type: "string",
          minLength: 1,
          description: "The original user task. A bounded summary is stored as the initial session title."
        }
      },
      required: ["task"],
      additionalProperties: false
  ...
  {
    name: "record_pushed_branch",
    description: "Record a branch and pushed head SHA after the provider has successfully pushed it.",
    inputSchema: {
      type: "object",
      properties: {
      ...
      required: ["sessionId", "branch", "headSha"],
      additionalProperties: false
    }
  },
  {
    name: "associate_pr",
    description: "Associate a pull request that the provider already created with its background Polygraph session.",
    inputSchema: {
      type: "object",
      properties: {
        sessionId: { type: "string", minLength: 1 },
      ...
      required: ["sessionId", "prUrl"],
      additionalProperties: false
    }
  },
  {
    name: "update_session",
    description: "Update the title or handoff description for the machine-owned background session.",
    inputSchema: {
      type: "object",
      properties: {
        sessionId: { type: "string", minLength: 1 },
  ...
  },
  {
    ...
    description: "Complete the background session. WARNING: current completion behavior closes OPEN and DRAFT pull requests.",
    inputSchema: {
      type: "object",
      properties: {
        sessionId: { type: "string", minLength: 1 },
        confirmCloseOpenPrs: {
          type: "boolean",
          const: true,
          description: "Explicit confirmation that OPEN and DRAFT pull requests may be closed."
        }
      },
      required: ["sessionId", "confirmCloseOpenPrs"],
      additionalProperties: false
    }
  }
];
var server = new Server(
  { name: "james-polygraph-background-spike", version: "0.0.0-spike" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (request.params.name) {
      case "background_session_start": {
        const task = args["task"];
        if (typeof task !== "string" || task.trim().length === 0) {
          return toolError("A non-empty task is required");
        }
        cliArgs = [
          "session",
          "background-start",
      ...
      case "update_session": {
        ...
        if (!title && !description) {
          return toolError("A title or description is required");
        }
        if (title)
      ...
      case "complete_session":
        if (args["confirmCloseOpenPrs"] !== true) {
          return toolError(
            "Completion requires explicit confirmation that OPEN and DRAFT pull requests may be closed."
          );
        }
        cliArgs = [
          "session",
    ...
    return toolError(
      "Invalid tool input. No Polygraph operation was performed."
    );
  }
  return runPolygraphCli(cliArgs);
});
async function runPolygraphCli(cliArgs) {
  const cliPath = fileURLToPath(new URL("./polygraph.js", import.meta.url));
  try {
    ...
    const detail = extractCliFailure(error);
    return {
      content: [
        {
          type: "text",
          text: detail ?? `Background Polygraph operation failed (exit ${exitCode}).`
        }
      ],
      isError: true
    };
...
function extractCliFailure(error) {
  if (!error || typeof error !== "object")
    return null;
  const output = ["stdout", "stderr"].flatMap((field) => {
    if (!(field in error))
      return [];
    const value = error[field];
    return typeof value === "string" ? [value] : [];
  }).map(redactMachineCredentials).join("\n").trim().slice(0, 16384);
  if (!output)
    return null;
...
function redactMachineCredentials(value) {
  const secret = process.env["POLYGRAPH_SERVICE_ACCOUNT_SECRET"]?.trim();
  const clientId = process.env["POLYGRAPH_SERVICE_ACCOUNT_CLIENT_ID"]?.trim();
  const encoded = secret && clientId ? Buffer.from(`${clientId}:${secret}`).toString("base64") : void 0;
  return [secret, encoded].filter((candidate) => Boolean(candidate)).reduce(
